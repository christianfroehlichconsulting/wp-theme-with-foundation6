...
if ( ! function_exists( 'cfcdesignsystem_start_cleanup' ) ) :
function cfcdesignsystem_start_cleanup() {

	// Launching operation cleanup.
	add_action( 'init', 'cfcdesignsystem_cleanup_head' );

	// Remove WP version from RSS.
	add_filter( 'the_generator', 'cfcdesignsystem_remove_rss_version' );

	// Remove pesky injected css for recent comments widget.
	add_filter( 'wp_head', 'cfcdesignsystem_remove_wp_widget_recent_comments_style', 1 );

	// Clean up comment styles in the head.
	add_action( 'wp_head', 'cfcdesignsystem_remove_recent_comments_style', 1 );

	// Clean up gallery output in wp.
	add_filter( 'cfcdesignsystem_gallery_style', 'cfcdesignsystem_gallery_style' );

}
add_action( 'after_setup_theme','cfcdesignsystem_start_cleanup' );
endif;
/**
 * Clean up head.+
 * ----------------------------------------------------------------------------
...
if ( ! function_exists( 'cfcdesignsystem_cleanup_head' ) ) :
function cfcdesignsystem_cleanup_head() {
	...
	add_filter( 'style_loader_src', 'cfcdesignsystem_remove_wp_ver_css_js', 9999 );

	// Remove WP version from scripts.
	add_filter( 'script_loader_src', 'cfcdesignsystem_remove_wp_ver_css_js', 9999 );

}
endif;

// Remove WP version from RSS.
if ( ! function_exists( 'cfcdesignsystem_remove_rss_version' ) ) :
function cfcdesignsystem_remove_rss_version() { return ''; }
endif;

if ( ! function_exists( 'cfcdesignsystem_remove_wp_ver_css_js' ) ) :

// Remove WP version from scripts.
function cfcdesignsystem_remove_wp_ver_css_js( $src ) {
	if ( strpos( $src, 'ver=' ) ) {
...
if ( ! function_exists( 'cfcdesignsystem_remove_wp_widget_recent_comments_style' ) ) :
function cfcdesignsystem_remove_wp_widget_recent_comments_style() {
...
if ( ! function_exists( 'cfcdesignsystem_remove_recent_comments_style' ) ) :
function cfcdesignsystem_remove_recent_comments_style() {
...
if ( ! function_exists( 'cfcdesignsystem_gallery_style' ) ) :
function cfcdesignsystem_gallery_style( $css ) {
...
if ( ! class_exists( 'CFCdesignsystem_img_rebuilder' ) ) :
	class CFCdesignsystem_img_rebuilder {
